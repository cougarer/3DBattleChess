...
public class ProtocolBytes:ProtocolBase
{
    ...
    public string GetString(int start, ref int end)
    {
        if (bytes == null)
            return "";
        if (bytes.Length-start < sizeof(Int32))   //总长度为10，start为7，你必须留出4个字节在存字符长度信息，所以return
        {
            return "";
        }
        Int32 strLen = BitConverter.ToInt32(bytes, start);//得到该小节的长度信息
        if (bytes.Length < start + sizeof(Int32) + strLen) //sizeof(Int32)+strLen 代表该小节的总长度信息（长度+信息）
        {
            return "";
        }
    ...
    public void AddInt(int num)//直接加在原数据的后面
    {
